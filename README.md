...
## 프로젝트를 통해 성장한 점
이번 프로젝트를 통해서 __처음 설계 단계에서 최대한 구체적으로 만드는 습관을__ 가져야겠다는 생각을 했습니다. 이번 프로젝트를 진행하면서 설계 단계에서 고려하지 못했던 부분이 많았던 것 같습니다. 특히 기능 구현 시간이나 다른 팀원과 관련이 있는 사항들을 __구체적으로 설계하지 못했기 때문에__ 개발 단계에서 수정해야 될 일이 많았고, 이전에 이미 구현한 기능들과 겹치는 경우도 있어 __수정하는데 시간이 더 오래걸렸던 경험이__ 있었기 때문입니다. 그래서 이후에 프로젝트나 협업을 진행하게 된다면 다른 팀원과 접점이 있는 부분을 먼저 체크하고 어떤 방향으로 구현할지 얘기를 많이 나누고 맡은 파트에서의 __기준이나 설계들을 잘 파악하고 계획해야겠다는__ 생각이 들었습니다.
